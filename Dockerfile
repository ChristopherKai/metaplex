...
RUN apt update && apt install nodejs yarn jq sudo -y
...
RUN cd /user/metaplex-foundation/metaplex/js/packages/cli && yarn install && yarn build && sed -i.backup -e 's/--no-bytecode//' package.json && yarn run package:linux
...
RUN cd /user/candy-machine-mint && yarn install && yarn build
